-- AgroBloque: fix completo de lectura para links de invitado.
-- Ejecutar en Supabase SQL Editor si el invitado entra pero ve la app vacia.
-- No habilita crear, editar ni borrar para invitados. Solo SELECT para anon con token valido.
...
begin
  ...
  select coalesce(
    g.permisos,
    '["buscar","alertas","historial","mapa","agenda","vivero","cosecha","ventas","inventario","fumigaciones","plan_nutricional","costos","contabilidad","reportes","compradores"]'::jsonb
  )
  into permisos_json
  from public.guest_access_links g
  where g.token_hash = digest_hex
